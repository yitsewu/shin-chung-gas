...
![瓦斯用量選擇欣中天然氣總用氣量，瓦斯流率留空](docs/images/energy-gas-usage.png)

**2. 加入費用**

往下捲動，選 **獨立實體以追蹤總成本**，再將「總成本實體 (TWD)」設為 **欣中天然氣 總費用**，最後按「儲存」。

![成本追蹤選擇獨立實體，總成本選擇欣中天然氣總費用](docs/images/energy-gas-cost.png)
...
資料來自欣中天然氣官網的「瓦斯費用暨度數明細資料」。使用六碼戶號及戶名查詢，沿用官網 Big5／CP950 編碼；目前流程不需 CAPTCHA。網站改版或新增驗證時可能需要更新整合。

身分保存在 HA 設定中；帳單與查詢紀錄保存在本機 Store，能源統計寫入 Recorder。GitHub 檢查使用維護者設定的 Actions Secrets，不會自動收集其他安裝者的身分或帳單。官網查詢協定使用含身分的 GET 參數，網站端可能記錄查詢；整合不輸出原始 URL、HTML 或 Cookie。
